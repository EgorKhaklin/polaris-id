...
8. **CI flakes look like failures.** Five known signatures: the runner's apt index
   ...
   the tree cannot be the cause), and the rolling drill's preflight finding no `app-green` in
   `docker compose config --services` seconds after the same job booted both colours healthy
   (`the blue-green overlay is not active`, 2026-09-16; a transient empty `compose config` on
   the runner). `python3 scripts/polaris-ship.py triage` names them;
