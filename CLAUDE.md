...
| C6 | Disclosure level is enforced server-side | Constitutional | Application and SQL functions: the form handler coerces the token id to NULL for zero-knowledge, the C2 CHECK refuses anything else, and every read path (the Atlas functions in `11_atlas.sql`, the event queries in `app.py`) nulls location for zero-knowledge rows; the redaction property suite attacks it | `check_c6_atlas_redacts_zk_location`, `check_redaction_adversary_is_not_flattered` |
...
| C8 | Every Atlas aggregate is bounded | Engineering | Application, into SQL: route-level clamps against the `_ATLAS_MAX_*` constants in `app.py`, passed as the limit the SQL functions apply. The SQL alone bounds nothing; the clamp is the guarantee | `check_c8_atlas_caps` (every caller-controlled count on every Atlas route) |
