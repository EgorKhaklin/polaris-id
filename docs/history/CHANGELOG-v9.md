# Changelog, version 9: v9.44 to v9.452

**Reader:** anyone tracing when and why a mechanism changed. **Job:** the entries of the root
[CHANGELOG.md](../../CHANGELOG.md) from 3 June 2026 (v9.44) to 12 September 2026 (v9.452),
moved here unchanged on 16 September 2026 so that the root file holds only recent ships.
Entries are never edited retroactively; a correction is a later entry, which may be in the
root file. Ships before v9.44 predate the changelog and are in the git log only.
...
contract verified an ID token's signature and never its audience or nonce, so a verifier ignoring
both, and therefore accepting a token minted for one relying party at another, passed all 71
cases. Four cases fixed it. This asks the same question of everything else, by measurement.

**`scripts/polaris-conformance-mutation-drill.py`** forces each decision field a verifier reports
to its permissive answer, the verdict an implementation that never performs that check would
give, and re-runs every published case. A field nothing notices is a check an integrator can skip
and still conform.

Its negative control forces the authenticity key the contract actually reads for each artifact.
**Three of nineteen were caught by no case at all:**

- **agent-proof** and **grant-revocation**: both cases expect `authentic: true`. The "impostor"
  case is genuinely signed by the wrong key, which is authentic and powerless by design, and the
  authority question belongs to `verify_agent_grant`. That separation is right. It also meant the
  signature itself was never exercised, so a verifier that skipped it conformed. For a grant
  revocation that matters plainly: a revocation nobody verifies is one anyone can mint, ending a
  grant that is still valid.
- **holder-proof**: one case, expecting `authentic: true`. The holder-chain refusals each fail for
  a reason that is not the signature (a stranger's key, a replayed nonce, a revoked binding), so
  forcing the signature check permissive left every case green.

Three tampered vectors, each generated by flipping sixteen bytes of the signature and then
**asserting the verifier refuses it** before writing the file, and three cases expecting
`authentic: false`. 82 cases now; both shipped verifiers pass all of them, the detached one
through `test_verify_conformance` and the SDK through `run_conformance.py --self`. The drill's
negative control reads 19 of 19.

### What the contract still does not constrain

88 fields, declared exactly rather than left to be rediscovered. The reason is structural: the
conformance adapter reduces most artifacts to `{authentic, fresh}`, so everything else a verifier
computes -- `issuer_matches`, `commitment_ok`, `revocation_checked`, `request_bound` -- is
invisible to the cases. Closing one means publishing a vector in which that field is the only
thing wrong, which is a case per entry and not one ship.

The declaration is checked **both ways**, which v9.423 taught: a new survivor fails the drill, and
so does a declared one that has become constrained, because a list that can only grow stops
describing anything. Both directions were tested by breaking them.

The drill also measures its own scope. It wraps every verifier with a recorder and runs the cases
to learn which 27 of the 40 the contract actually reaches; the other 13 are named and excluded.
Its first draft skipped that and reported `verify_signed_document.authentic` as unconstrained,
when the contract reads `document_authentic` and constrains it correctly. A survivor list with
false entries is a list nobody finishes reading, which is the same failure v9.428 fixed in the
verification plan.

`check_conformance_contract_constrains` requires every artifact the contract judges on
authenticity to carry a case where the signature is bad, and requires the drill to declare its
gaps, fail in both directions, keep its negative control, and run in CI. Run against the contract
as it stood before this ship it reports exactly the three artifacts that were wrong. Nine
discriminations.

One of those discriminations failed on the first run for a reason worth recording: the fixture put
its marker in a `#` comment, and `_read` strips comments on purpose. The check was right; the test
was asking it to find something that had been removed before it looked.

**249 invariant checks. 82 conformance cases.**
