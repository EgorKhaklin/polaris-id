...
\begin{itemize}
  ...
  \item The membership prover runs on the host; there is no holder-side key, no on-device proving,
    no scoped nullifier, no pairwise or blinded presentation. The interim credential during a
    recovery cool-down is not built. Attestations are recorded by an operator, not cryptographically
    signed by the attesting agency.
  \item Anchor verification for timestamps lives in the detached verifier only; the SDKs verify a
    timestamp and its head as artifacts and not its inclusion or witnessing.
  \item The one audit-of-record instance not fully enforced at the schema, \entity{RecoveryRequest},
    rests on procedure discipline.
\end{itemize}
