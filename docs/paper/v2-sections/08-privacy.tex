...
state, so a relying party that needs single use issues a challenge. There is no per-verifier
nullifier, so the proof cannot by itself enforce \emph{one human, once} (Section~\ref{sec:agents}).
The prover runs on the Polaris host, not on the holder's device. And the circuit over the proof
library has had no external cryptographic audit; the repository's soundness ledger says not to
protect real identities with it as shipped. \sExt
...
\subsection{The thing that does not exist}

Polaris is a full-credential presentation system. A presentation carries a stable \code{token\_value},
and the login token's subject is the credential's hash. Two relying parties who both see a credential
can therefore join their logs by that value, with no name required, and nothing in C2 or C6 stops
them. Even without the value, any stable handle shown to every verifier, a reused public key, a
reused membership proof, is the same correlator. Polaris does not implement pairwise or per-verifier
identifiers, blinded or derived presentations, one-time presentation tokens, or anonymous
credentials, and the repository records this as a permanent, documented property rather than a
pending feature. \emph{Unlinkable by default} means the issuer's zero-knowledge-mode records. It
does not mean cross-verifier presentation. The future presentation technologies that would change
this are discussed in Section~\ref{sec:research}; none is built. \sImpl{} as a stated limitation.
