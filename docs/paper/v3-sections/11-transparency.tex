...
\section{Watching the watchers: logs, witnesses, the split view and the public report}
\label{sec:transparency}

\motif{A transparency log is not protection from a split view.}

\analogy{Each evening the watchtowers publish what the hall recorded that day, and cannot quietly take it back. Witnesses outside the wall copy every page and compare notes, and a tower that shows two visitors two different pages is caught when the witnesses meet.}
...
\figfloat{transparency}{The log, the monitor, the witnesses, the gossip pool and the equivocation
...
\subsection{The public report: saying out loud how often the power was used}

The logs prove that history was not rewritten. They do not tell the public what the history
contains, and one part of it matters more than the rest: an identity authority asks to be trusted
with the power to pull one named person's entire verification history under a warrant, and every
control around that power, the warrant, the role check, the disclosure model, is invisible from
outside. The authority can publish a transparency report that says how often it
used that power, built from the audit-access record the read now leaves (Section~\ref{sec:privacy}).
Building it began by asking what was recordable and finding the answer was nothing, because the read
had left no trace of itself. Small counts are withheld rather than published as zero, because a
report with a cumulative total lets a reader subtract one period from the next and recover the cell,
so the report carries no running total to subtract from; a drill on every push attempts exactly that
recovery and must fail. The report is a surface where the authority makes its own most invasive act
countable by strangers. It says nothing about an access that was never recorded, and the review
packet names that as a limitation of every authority's own log. \sImpl
...
\qa{What it is}{Three RFC-6962-style logs over append-only tables; signed heads; consistency and inclusion proofs; a monitor; witnesses with cosignatures, a threshold and gossip; the equivocation proof; external-ledger publication with receipts; a public report of the authority's own warrant reads.}
\qa{Why it exists}{A signed object proves what was said; only a watched sequence proves that what was said was not later unsaid; only compared notes prove it was said the same way to everyone; and only a published count lets the public ask how often the watched thing happened.}
\qa{What it trusts}{The log's key for the head; the witnesses' keys and their independence, which is a deployment fact; the authority's own record of its reads.}
\qa{What it refuses}{A head with no consistency proof from the last one; a head not cosigned by the threshold; a receipt the ledger did not record; a second head at a size already seen; a report from which a small count can be recovered.}
\qa{What it can see}{Roots and hashes, and counts of warrant reads. Never a person, never a payload.}
\qa{What it cannot see}{On its own, a split view: that is what the witnesses are for. An access that was never recorded.}
\qa{If it fails}{A rewrite fails every monitor; a fork is a non-repudiable proof; a dishonest witness is one of $K$; a dropped published head fails the ledger's own consistency; a recoverable small count fails the report drill.}
\qa{Checked by}{\code{check\_transparency\_log}, \code{check\_transparency\_gossip}, \code{check\_transparency\_publication}, \code{check\_timestamp\_transparency}, \code{check\_transparency\_program}, \code{check\_audited\_reads\_are\_logged}, the canonical-equivalence oracle for the signed head.}
