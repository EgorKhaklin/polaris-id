...
\motif{A verdict from the issuer is not a verdict anyone can check.}

\analogy{A townsperson's papers can be checked at any inn on any road, by anyone who holds the town's published seal, without sending a messenger to the hall. The town is never asked to vouch for its own seal.}
...
library: no Polaris code, no database, no network unless asked to fetch status. It is a package on a public registry, at \code{1.0.0-rc.3} (each
package carries its own version, separate from the system's), a release candidate, with \code{0.1.0} beneath it since 15 September 2026, published by the
...
wallets Polaris has never met. It is an OpenID4VP 1.0 verifier (OpenID4VP is the standard protocol by which a digital wallet presents
a credential) under the High Assurance Interoperability Profile (HAIP), a role for which the OpenID Foundation's conformance suite has a test plan, \code{oid4vp-1final-verifier-haip-test-plan}, in which \emph{the suite signs the
credential}, so Polaris's own ML-DSA issuance is not involved. \code{polaris-oid4vp} is that
verifier: it checks an SD-JWT VC (a signed credential whose fields can be disclosed one by one) signed with
ES256 (a classical elliptic-curve signature), with holder key binding, delivered as an
encrypted response the wallet posts back to the verifier (\code{direct\_post.jwt}), and it refuses with a named reason, one per negative
...
implementation of CBOR, the standard's binary encoding, proving it rather than the one that wrote the bytes; the reader cannot verify the
