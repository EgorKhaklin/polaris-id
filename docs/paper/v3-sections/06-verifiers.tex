...
the same plan against \code{1.0.0rc7} was certified (24 September 2026). A published corpus agrees
with both signature witnesses on the primitive. None of the three is an audit or a pilot, and the
one certification covers one package version in one role.}{fig:outside-witnesses}

Everything above is Polaris verifying Polaris. One path has been exercised by an external
implementation, chosen because it is the one an outside wallet actually speaks. Under the operating contract the
...
and a Foundation reviewer sees them only at certification submission. REVIEW is not PASSED. The
certification came later and separately: on 24 September 2026 the same plan was run once more,
against the \code{1.0.0rc7} artifact on PyPI and in one plan as a certification package requires,
all eleven modules finished without failure, and the Foundation approved the request and lists
\code{polaris-oid4vp 1.0.0rc7} as OpenID Certified to the OpenID4VP 1.0 + HAIP 1.0 Verifier profile.
That is a self-certification the Foundation reviewed and published, for that version in that role:
not an endorsement, not an audit, and not a statement about the rest of Polaris. The first write-up of the run said eleven of
...
modules, one patched to accept everything by all seven negatives. \sWit{} for the profile, and
certified for that one package, version and role.
...
usable for signing; eleven conformance modules, 142 package tests, a mutation drill over every
...
has not been exercised from outside: the specification, the cases and two SDKs exist, and no implementation
