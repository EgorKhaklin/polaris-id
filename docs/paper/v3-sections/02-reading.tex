...
environment? How does it connect to the layer inside it and the layer outside it? The answer to what
checks it is the only one kept out of the cards: the checks and drills behind each card are listed
in Appendix~\ref{app:status}, so the body reads without them. A reader who can answer those
questions for every ring has the mental model this paper exists to build.
...
below. The analogy describes arrangement and sight lines, not political centralisation: Polaris is
federated per authority, with no central database, root or verification service. It will recur in
a shaded box like the one above, and every
...
\figfit{0.55}{town-map}{The walled town, with the real component beside every civic name. The register
