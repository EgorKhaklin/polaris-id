...
outside the repository have done with the rings inside it, and at \code{1.0.0-rc.1} that is one
...
cannot repeal by a vote of its officials. And at the main gate, since this version, a traveller from
a town nobody here built has presented papers and been let through, after first being turned back for
a flaw in the town's own seal that none of its clerks had noticed.}
...
recurrence is followed by the mechanism it stands for. The traveller is walt.id's wallet, the gate is
the OpenID4VP verifier, and the flaw in the seal was a request-signing certificate with no
\code{digitalSignature} key usage; Section~\ref{sec:verifiers} gives all three by name.
