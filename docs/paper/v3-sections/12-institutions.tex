...
\motif{An exchange between institutions is not proof that it was authorized.}

\analogy{When one town's hall answers another's request, it hands back a receipt saying that a lawful exchange took place, under whose authority and when, without saying what the letter contained. The receipt proves the exchange, not its contents.}
...
One thing the registry does not carry is recorded in Section~\ref{sec:signatures}: the accepted
algorithms appear as a bare list of names with no status, so a verifier reading it cannot learn that
one has been deprecated. \sImpl
...
Authorization at the gateway is directional. The responding agency itself must
...
evidence is the signed statement in the requester's hands. A caller may choose
...
A holder in Polaris may hold a key (Section~\ref{sec:agents}), and a holder need not, so the
document-signing model stays notarial: the issuing authority signs on behalf of a holder who has just
proved possession, exactly as it asserts status for one, and records who by hash. The authority's
signature says that a holder of this credential, active at this instant, asked it to sign this
digest, and the embedded evidence lets anyone check that claim later. \sImpl
...
Anchor verification is in the detached verifier and both SDKs, so an implementation that imports no
Polaris code can decide long-term validity. \sImpl
