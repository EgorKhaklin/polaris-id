...
\emph{this authority issued this token to this person.} Since \code{v9.349} the claim may also bind a
...
Version 2 recorded the hardware card as modelled and not manufactured. That is still true of the
silicon, and false of everything around it. Since \code{v9.366} the card is an encoding with a
reference codec and published vectors (\code{polaris\_card/card\_profile.py}), a software token that
...
configuration it is required to fail. One more thing is true of it, and Section~\ref{sec:senses}
tells it in full: when the index was dropped to see what would notice, the suite that exists to prove
...
terminal states or into \code{DORMANT} when a successor takes over. The legal transitions are
enforced by the trigger \code{trg\_token\_state\_machine}, and every transition writes a lifecycle
event automatically.}{fig:lifecycle}

Version 1 recommended enforcing the transitions in a trigger and left it as future work. The
...
edges of the lifecycle, and each has grown a record of what it rested on since Version 2.
...
\begin{description}
  \item[Entering: tiered enrolment, and what an enrolment rests on.] Every person has an enrolment
    ...
    but unhelped by any function, and it is audited when done. Until \code{v9.371} the system could
    issue a credential and had no way to say how the person had been proven to be who they claimed.
    \entity{EnrollmentProofing} now records one proofing event and \entity{EnrollmentEvidence} what
  ...
  \item[Leaving: issuer discretion, as a decision that survives being changed.] The worst failure of
    ...
    required, recorded in the audit trail. The bound converts a surprise into a trend that reporting
    can see; it does not stop slow abuse under the ceiling, and says so. Since Version 2 the settings
    around this power have become records: the ceiling itself (\entity{IssuerDiscretionPolicy}) can
    ...
    relying party's bar, which could be lowered with nothing recorded anywhere until \code{v9.425},
    is now recorded with the same discipline, weakenings flagged and reasons required. \sImpl
  \item[Returning: the recovery ceremony.] A fire takes the wallet and the reserve together. Recovery
    ...
    way under a trigger since \code{v9.347}, and the four-eyes rule, the cool-down and the three
    channels are refusals a stored procedure makes that a mutation drill now proves the tests would
    notice losing (Section~\ref{sec:walls}). The holder is without identity for the cool-down; the
...
\subsection{Duress, and what the word is allowed to mean}
...
Version 2 called the system compulsion-resistant. Version 3 does not, because the repository's own
assessment of the mechanism against the three coercers the operating contract names concluded that
the word was not supported, and the vocabulary was weakened to \emph{duress-aware} on every outward
surface, with a check that refuses the strong word's return. Against a casual coercer, one who does
