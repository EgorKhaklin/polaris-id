...
\caption{The ten constraints at \code{1.0.0-rc.1}, their tier, and the object that enforces each. The table is kept honest against the code by \code{check\_c1c10\_objects\_resolve}, which fails the build if a named object no longer exists.}
...
\figfit{0.635}{districts}{The 45 canonical tables of \code{01\_schema.sql} at \code{1.0.0-rc.1}, as rings
...
party. A migrated deployment holds seven more tables (the migration registry, operator sessions and
credentials, the audit-access log, and Athena's three curated tables) and five partition
...
an append-only trigger at \code{1.0.0-rc.1}, the new ones being the records that decisions about
authorities, operators, relying parties and enrolments now leave behind, and since \code{v9.412} C1 is
tested once per table that claims it: a table-driven suite reads the append-only triggers out of the
catalogue rather than out of a list somebody wrote, so a new append-only table cannot simply go
untested. \sImpl

\figside{town-map}{The walled town, with the real component beside every civic name. The register
...
repository something about its own test database at \code{1.0.0-rc.1}: the seed that reloads the
...
Above the schema sits a flat layer of 272 plain functions, each of the form
...
Every one of these is now covered, and every one of these measurements is now a drill that runs on
every push with a negative control, so that a result of zero survivors is distinguishable from a
harness that ran nothing. Eight such drills exist at \code{1.0.0-rc.1}: over the checks, the
constraints, the triggers, the procedures, the zero-knowledge witnesses, the conformance contract, the
two reference verifiers and the OpenID4VP verifier. Section~\ref{sec:senses} carries the record.
\sImpl

\begin{layercard}{Layer card: the walls}
\qa{What it is}{PostgreSQL 16: 45 tables, 42 triggers, sixteen stored procedures that implement every state-changing use case, the append-only audit tables, the retention policy as data.}
