...
authorities, operators, relying parties and enrolments now leave behind, and since \code{v9.412} C1 is
...
\figpage{town-map}{The walled town, with the real component beside every civic name. The register
...
fixture in the specific way the check claims to catch and asserts that the check turns red, and,
since \code{v9.405}, also asserts that the check passes on the same fixture unbroken, because a test
...
surfaces a check quantifies over, and the moves of code between modules. Section~\ref{sec:senses} carries the record.
\sImpl

\begin{layercard}{Layer card: the walls}
\qa{What it is}{PostgreSQL 16: 45 tables, 42 triggers, sixteen stored procedures and four use-case functions that implement every state-changing use case, the append-only audit tables, the retention policy as data.}
...
\qa{What it trusts}{The database engine and its owner; the correctness of the constraints as written, which is why each is now mutation-tested.}
...
\qa{If it fails}{A dropped trigger, a lowered retention floor or a cascade fails the build through the checks; a constraint whose loss the suite would not notice fails the mutation drill; a database owner with a shell has larger options than any trigger, which the threat model states rather than hides.}
