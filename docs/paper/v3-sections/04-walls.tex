...
argument for treating them as a graph rather than a list. Athena, the read-only layer that describes the
structure of authority (Section~\ref{sec:cognition}), holds the map from each constraint to the exact live mechanism that enforces it, and a check fails
...
strict or bounded, so that each has a named test that answers for it, and an undeclared one fails
the build. \sImpl
