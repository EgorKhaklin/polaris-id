...
misread, and Version 3 says it more bluntly than Version 2 did. Without the real-signing flag and a
...
integration. It is guarded, since \code{v9.280} a production boot fails closed unless real signing is
...
saying so in the same breath. The history is recorded rather than hidden: at \code{v9.58} the
headline post-quantum claim was, at the data level, a hardcoded string, and the real signing module
was an unused island until it was wired into issuance. \sImpl
...
says so beside every two-witness claim. Since \code{v9.419} the witnesses are also mutation-tested:
the claim that two implementations must agree is now a claim about the tests as well as the code.
...
This subsection is new, and it replaces a word. Version 2 called Polaris post-quantum on its cover.
An outside argument, that the phrase is empty because nobody knows how large quantum computers will
get, attacked the wrong thing, and the part it got right was sharper than its framing: ML-DSA-65's
security does not rest on a hardware prediction, it rests on Module-LWE and Module-SIS being hard
...
the two witnesses at issuance. So the engineering was not the problem; the wording was. What the
repository can demonstrate is \emph{algorithm agility under an audited migration path}: C7 makes the
...
client-to-edge key exchange for modern clients and, since \code{v9.404}, the hop from the
application to the connection pooler are post-quantum, each of the last two read off a real
handshake rather than inferred from a base image, which is how the posture ledger had once called
one of them classical while it was not. The hop from the pooler to the database, the certificate
signatures, and the operator's WebAuthn credentials remain classical, each gated on a third party (a
