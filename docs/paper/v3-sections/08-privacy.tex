...
the epoch has a floor: since 25 September 2026 an epoch smaller than the minimum anonymity set (a
database setting, twenty unless set; the notional sample sets one and says so) is refused rather
than closed smaller, and a proof against an older one below it answers \emph{privacy unavailable}
...
size its signed checkpoint states. Before that date an epoch of one member closed, and a proof against it hid nobody.
...
The proof carries a per-verifier nullifier,
\code{Poseidon(secret\,||\,scope\,||\,epoch)}, so a relying party can enforce \emph{one enrolled
...
hidden behind a procedure or a view. The authority-structure layer, Athena, is forbidden by
