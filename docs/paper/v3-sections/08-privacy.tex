...
zero-knowledge events and asserts its success rate is no better than chance, and since \code{v9.408}
the assertion is shaped so that a weak adversary cannot flatter the result: two weaknesses in the
adversary had been invisible to the old shape. The claim is scoped precisely. It covers
...
row for the online status check. The red edge is the stated limitation: on a plain presentation the
...
epoch, a Merkle root over the active-token set, and a holder can produce a Plonky2 proof on their own
...
of fourteen covers the schema's ten-thousand-leaf epoch cap, so the anonymity set is a full epoch.
On the reference machine a proof takes about 35~ms to make and about 13~ms to check, and the verdict
is two-witnessed at the statement level. The proof carries a per-verifier nullifier,
\code{Poseidon(secret\,||\,scope\,||\,epoch)}, so a relying party can enforce \emph{one human, once}
...
not to protect real identities with it as shipped. The proof library itself was re-evaluated against
its successor at \code{v9.361} and kept, with the triggers that would reopen the question recorded.
\sExt{} for the audit.

\subsection{Bounded, not permanent, and what bounded was found to mean}

Polaris is a full-credential presentation system, and until \code{v9.353} the consequence was flat:
the login token's subject was the credential's hash, the same value at every relying party, so two
of them could join their user tables exactly and forever. The subject and the presentation handle
are now derived under the relying party's own scope, stable where an account needs it and
...
person's whole verification history under a warrant, left no record of its own use for as long as it
existed, because the read sat behind a stored procedure and nothing looking for a \code{SELECT}
found it; since \code{v9.382} it writes the same row, and a structural rule extends that to any read
hidden behind a procedure or a view. The authority-structure layer, Athena, is forbidden by five
checks from referencing any natural-person table or column (Section~\ref{sec:cognition}). And
...
\qa{If it fails}{An identifier on a zero-knowledge row is unstorable; a cap removed fails the build; a person reference in Athena fails five checks; an adversary weakened to flatter the result fails the property suite's own control. Cross-verifier correlation on a plain presentation is not a failure; it is the documented boundary.}
\qa{Checked by}{The C2 CHECK and the C2 model, \code{check\_c6\_atlas\_redacts\_zk\_location}, the redaction property suite, \code{check\_redaction\_adversary\_is\_not\_flattered}, \code{check\_pairwise\_presentation}, \code{check\_scoped\_nullifier}, \code{check\_audited\_reads\_are\_logged}, the ZK second-witness differential, \code{check\_athena\_no\_person}.}
