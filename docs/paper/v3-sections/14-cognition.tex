...
\motif{Watching the system is not watching the people in it.}
...
\figfloat{cognition}{The cognitive layers. Atlas sees what the system is doing; Athena understands
...
red line is the person-prohibition: nothing above it models a natural person, and
\code{check\_athena\_no\_person} fails the build if a person-level column or traversal appears in the
authority layer.}{fig:cognition}
...
server-side aggregate, never the raw events: at most five thousand cluster summaries for a viewport, however fine a grid the client asks for,
...
actually stream. The benchmark times every bounded aggregate at a tenth of the
event stream and at all of it, minutes apart on the same hardware, and grades each against the
row-count growth, because a timing at one scale says an aggregate was fast once and cannot tell a
roll-up that tracks the rows from one going quadratic; every aggregate grows slower than its data,
and the benchmark exits non-zero if one ever outruns it. \sImpl
...
\subsection{What a learning layer may never learn}
...
should be preferred to the model. Nothing of it is built; under the operating contract of
Section~\ref{sec:research} nothing of it will be until an outside party needs it. \sFut

Over all of these layers stands the constitution: the vocation and C1 to C10, enforced in the schema
and pinned by the checks. It is not a component; it is what every component is constrained by.
...
\qa{What it refuses}{Any person-level object, handle, traversal, score or action; an unbounded read; an aggregate that grows faster than its data; a model where a rule would do.}
...
\qa{If it fails}{A person reference anywhere in Athena's SQL fails \code{check\_athena\_no\_person} and stops the build, measured by adding one (the other Athena checks guard other properties and stay green); an unbounded aggregate fails C8; a learning system that scored people would violate the vocation and is refused before it is designed.}
\qa{Checked by}{\code{check\_athena\_no\_person}, \code{check\_athena\_read\_only}, \code{check\_athena\_functions\_bounded}, \code{check\_athena\_non\_sovereign}, \code{check\_athena\_rule\_enforcement\_resolves}, \code{check\_atlas\_console}, \code{check\_benchmark\_measures\_growth}, the C8 caps.}
\qa{Now or later}{Atlas and Athena now. Metis after an assessment, and only if an outside party needs it; Myrmex is a proposal in this paper.}
...
wrong thing. The next section is that lesson, and this version has a great deal more of it to tell.}
