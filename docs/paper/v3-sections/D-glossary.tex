...
Terms come first, in alphabetical order. The abbreviations the text uses follow them, each expanded,
and for a standard or an organisation, what it is.
...
\begin{description}
  ...
  \item[Athena.] The read-only layer that holds the structure of authority (agencies, keys, contexts, trust
    and rules, and what enforces each) and the constitution as data. It may not model a person.
  \item[Atlas.] The bounded operational console: aggregates of what the system is doing, capped by the
    application, never the location of a zero-knowledge event and never a person.
  ...
  \item[C1--C10.] The ten hard constraints of the constitution: five constitutional rights guarantees (C1, C2,
    C3, C6, C10) and five engineering constraints that keep them honest under load and attack. Each
    is named in the section that enforces it.
  ...
  \item[Dyno.] The benchmark that runs in continuous integration and publishes the throughput and latency
    it measured on that machine; a number cited as "dyno" was measured there, not estimated.
  ...
  \item[Lab.] The part of the repository whose job is to try to falsify the claims that make Polaris
    different (linkability, duress, the cryptographic migration). It carries no product version.
  ...
  \item[Metis.] A proposed layer that would learn the behaviour of Polaris itself: anomalies, capacity,
    regressions, release risk. Not built.
  \item[ML-DSA.] The lattice-based signature scheme standardised as FIPS 204; parameter sets 65 and
    ...
    security.
  \item[Mosca's inequality.] The comparison of the years data must stay secret plus the years a migration
    takes against the years until a quantum computer can break today's signatures. When the first
    sum is larger, what is signed today is already at risk.
  ...
  \item[Myrmex.] A proposed debugging system outside Polaris: specialised agents that attack it independently
    and leave reproducible evidence on a shared landscape, coordinated by stigmergy. Not built.
  ...
  \item[Poseidon.] A hash function designed to be cheap inside a zero-knowledge circuit. Polaris uses it for
    epoch leaves and nullifiers.
  ...
  \item[Posture ledger.] The repository's record of each cryptographic dependency with its threat and its
    exposure, including the gaps, such as the hash-based signer that is registered and not wired.
  \item[Readiness ledger.] The repository's bound on what it may claim: what a production deployment would need
    that the repository does not provide, and the decisions it leaves to a deploying organisation.
  ...
  \item[Review packet.] The document handed to an outside reviewer: what each subsystem is supposed to
    guarantee, what holds it, and where to spend attention first.
  ...
  \item[Soundness ledger.] The record of what the membership proof's soundness rests on and what was
    measured about it.
  ...
  \item[Stigmergy.] Coordination through traces left in a shared environment instead of a central plan, as in
    an ant colony. The proposed Myrmex would work this way.
  ...
  \item[Version numbers.] The tree counted builds as \code{v9.N} until mid-September 2026 and
    then took release-candidate numbers, \code{1.0.0-rc.N}, which move only for a change seen from
    outside. This map is drawn at \code{1.0.0-rc.7}; a \code{v9.N} beside a measurement says which
    build it was taken on. The packages carry their own version numbers.
  \item[Witness.] An independent party that cosigns log heads it found consistent and refuses a fork.
  \item[Wycheproof.] Project Wycheproof, a public corpus of cryptographic test vectors, including inputs a
    correct implementation must reject. Polaris runs its ML-DSA vectors under both witnesses.
