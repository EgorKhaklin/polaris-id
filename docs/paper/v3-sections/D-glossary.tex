...
\begin{description}
  ...
  \item[Gossip.] Witnesses sharing the log heads they were shown so that a split view is detected by
    comparison.
  \item[HAIP.] The High Assurance Interoperability Profile of OpenID4VP, which pins the credential
    format, the signature algorithm and the response mode a wallet and a verifier must use.
  ...
  \item[Release candidate.] The version \code{1.0.0-rc.1}: every condition the operating contract
