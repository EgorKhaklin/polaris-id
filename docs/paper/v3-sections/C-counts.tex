...
Mutation drills & 12 & Files matching \code{scripts/\allowbreak{}polaris-*mutation-drill*}: checks, constraints, triggers, procedures, the zero-knowledge witnesses, the conformance contract, the two SDKs, the OpenID4VP verifier, the application, coverage, the constitution, and moves between modules \\
...
Signed wire formats in the specification & 27 & Distinct \code{polaris-*/1} strings in \code{docs/\allowbreak{}reference/\allowbreak{}WIRE-SPEC.md} \\
...
Migrations & 45 & Files matching \code{polaris\_sql/\allowbreak{}migrations/\allowbreak{}*.up.sql}, each with a revert \\
...
measured it. The application baseline has not been re-run since \code{v9.191} and is cited with
that stamp. The dyno was re-run on 23 September 2026 against the tree at \code{1.0.0-rc.7}, with the
prover rebuilt from it, and the re-run found that its proof measurement had measured nothing since
\code{v9.354}: it still sent the prover an input the prover no longer accepted, printed \emph{not
measured here}, and exited cleanly, so its CI step stayed green. The signature rows moved by a few
per cent; the proof row fell from 35~ms to 25~ms, and is not like for like, because the statement
proved changed at \code{v9.354}. A half that does not measure now fails the step. The reference machine throughout the repository is an
