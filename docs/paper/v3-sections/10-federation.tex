...
six. A review at \code{v9.364} asked whether one authority's operators could see another's data in
such an instance, and found that they could: an operator account carried no authority binding at all,
and sixteen of the seventy-four operator routes read credential or holder data with no
issuing-agency filter, the token list plainest among them, selecting every token joined to every
holder with no notion of whose it was. Operators are now bound to an agency and every such route
filters on it. What the review also recorded is the part no filter fixes: in a shared instance the
database owner, the backup and the schema are shared, so isolation inside one instance is a
courtesy the application extends, not a boundary the schema holds. The topology the design record
