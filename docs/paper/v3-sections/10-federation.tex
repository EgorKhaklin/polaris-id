...
\motif{Federation is not institutional independence.}

\analogy{Other towns keep their own halls and their own dies. A road between two towns runs one way and for one purpose, a town honours another's traveller only because it has said so in writing, and no town's trust passes on to a third.}
...
\figfloat{federation}{Two authorities and one directional attestation. B attests A's key in one
...
\subsection{One instance, many authorities, and the part policies cannot fix}

A Polaris instance can hold many agencies; the schema permits it and the shipped seed demonstrates
six. Inside such an instance an operator is bound to one authority, and the database, not the query,
keeps the others' credentials and holders out of that operator's sight: row-level security hides
them from a bound session, the views run with the caller's rights, and the routines that must see past
the binding to enforce a rule do so as the owner and return nothing the binding would hide. What no
binding changes is that the database owner, the backup and the schema are shared, so against the owner
isolation inside one instance is not a boundary. The topology the design record
chose is that each authority runs its own instance, and a shared instance is a deployment decision
whose cost is stated. \sImpl{} for the binding and the filters; \sExt{} for a boundary between
authorities, which is a deployment.
...
independent implementation, written by someone else from the specification, interoperates on the
native protocol. The first requires deployment by institutions that are not the author. The second
requires an external team, and has not happened; the one outside implementation that has spoken to
Polaris did so over OpenID4VP (Section~\ref{sec:verifiers}), not over these formats. The repository
draws this line itself: protocol independence, independently operated institutions and independent
third-party implementations are three different claims, and only the first is demonstrated. \sExt

\begin{layercard}{Layer card: federation}
\qa{What it is}{Per-authority roots, directional per-context attestations, the signed manifest, epoch checkpoint and revocation feed, the aggregate status bundle, cross-authority zero-knowledge membership, per-authority operator isolation, and the two-instance drill.}
...
\qa{What it refuses}{Transitive trust; a central service in the verification path; a manifest not signed by its own declared anchor; an aggregator's word for a member's status; a callback the issuer could log; an operator reading another authority's holders.}
...
\qa{If it fails}{A forged manifest fails self-consistency; a forked epoch chain or a rolled-back feed is non-repudiable evidence; a compromised issuer key is retired from an instant by a trust list the issuer does not control; a shared instance's owner is outside what any filter can bound.}
\qa{Checked by}{\code{check\_federation\_topology}, \code{check\_inter\_authority\_protocol}, \code{check\_epoch\_revocation\_propagation}, \code{check\_cross\_authority\_zk}, \code{check\_federation\_two\_instances}, \code{check\_exchange\_trust\_directional}, \code{check\_route\_guards\_are\_derived\_not\_listed}.}
