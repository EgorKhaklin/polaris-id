% E-from-v1.tex: what Version 2 kept from Version 1, and what it corrected; Version 3 keeps the same.
...
kept, what it corrected, and what it retired, so that a reader of both can reconcile them. Version 3
keeps every decision below unchanged; what it changed against Version 2 is Appendix~\ref{app:from-v2}.
...
\begin{itemize}
  ...
  \item The argument that post-quantum signing belongs on the first day, through the secrecy
    ...
    here and stand in Version 1 in full; what Version 3 narrows is the \emph{claim} made about the
    signatures, not the argument for choosing them (Section~\ref{sec:signatures}).
  \item The comparison of design properties against deployed systems, which this paper does not
...
\begin{itemize}
  \item Twelve tables became thirty-six at Version 2 and forty-five at Version 3. The three
    subsidiary entities of Version 1 (device bindings, ledger anchors, the revocation list) remain;
    the rest of the growth is described ring by ring in Section~\ref{sec:walls}.
  ...
  \item Version 1 modelled seven verification contexts with per-context biometric requirements and a
    physical token with on-device biometric binding. The contexts remain; the physical token is now
    an encoding, an emulator and a verifier device, and remains unmanufactured.
