...
\begin{tikzpicture}[node distance=0pt]
  ...
  \node[futbox, anchor=north west, font=\scriptsize, text width=5.4cm, align=left]
    ...
    значение токена удостоверения, поэтому вердикт называет соотносимость \emph{exposed}.
    Лишь путь с нулевым разглашением есть \emph{bounded}.};

  % --- the property the whole design exists for ----------------------------------
  \node[anchor=north, font=\scriptsize\itshape, text=navymid, text width=12.6cm, align=center]
    at (5.3,-5.15) {Отзыв полномочия не отзывает человека. Держатель подписывает
