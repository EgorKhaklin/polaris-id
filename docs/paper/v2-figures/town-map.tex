...
\node[law] (law) at (0,6.6) {The laws above the town: the vocation and C1--C10 (MISSION.md), enforced in the schema, pinned by 191 checks};
\draw[flowfaint] (law.south) -- (0,5.5);
% The wall (schema constraints).
\draw[wall] (-5.4,-4.8) rectangle (5.4,5.0);
\node[civic, anchor=south east] at (-1.5,5.15) {the wall: triggers, checks, unique indexes};
% Districts.
...
% Gates on the wall.
\node[gate] (g1) at (0,5.0) {issuance gate\\uc1\_issue, signing};
\node[gate] (g2) at (5.4,1.2) {verification gate\\/api/v1/verify};
\node[gate] (g3) at (0,-4.8) {exchange gate\\/api/v1/exchange};
\node[gate] (g4) at (-5.4,1.2) {login gate\\auth broker, PKCE};
...
\node[extbox, minimum width=2.6cm] (wit) at (-8.5,-3.4) {independent witnesses\\[-1pt]{\tiny cosign heads, gossip,}\\{\tiny refuse a fork}};
...
% The holder outside.
\node[humanbox, minimum width=2.4cm] (holder) at (-8.5,1.2) {holder\\[-1pt]{\tiny wallet file, presentation}};
\draw[flow] (holder.east) -- (g4.west);
\node[font=\tiny\itshape\color{steelgray}, anchor=north, text width=3.0cm, align=center] at (-8.5,0.3) {enters by possession; a duress code looks like consent to the coercer};
\end{tikzpicture}
