...
| A-12 | The check layer detects its own violations | `check:controls_as_attacks`, `check:attacks_run` | a check that passes against a tree where the property it names is false |

A-12 is the one to start with. Every other guarantee on this page is believed because a check
says so, and the checks are written by the same hand as the code. If a check can be made to pass
on a broken tree, everything above it is worth less than it looks.
