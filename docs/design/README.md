...
| [auth-broker.md](auth-broker.md) | The auth broker (P8.4): authorization code + PKCE, a holder authenticating by possession, an issuing-agency-signed ID token whose subject is a credential hash, ZK step-up, duress served identically, and the guards that keep the authority from holding a record of who logged in where |
