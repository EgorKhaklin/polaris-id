...
#: Set only by the CLI, only under --dev-placeholder. A declared mode that did not change
#: what the process does is a label, not a mode: measured on the first run of the gate, a
#: genuine ML-DSA-87 pack verified TRUE with real cryptography and was stamped
#: DEV-PLACEHOLDER. The stamp has to describe what happened, so the flag turns the real
#: backends off rather than merely announcing that they are not being trusted.
...
def verify_pack(pack: dict, anchor_keys=None) -> dict:
    ...
    if primary is None and witness is None:
        verdict["note"] = ("this run declared --dev-placeholder, so no real backend was "
                           "consulted: NOTHING here is authenticated"
                           if _DEV_PLACEHOLDER_MODE else
                           "no ML-DSA-65 verifier available: install liboqs-python "
...
def verify_dir(dir_path) -> int:
    ...
    for p in paths:
        ...
        if not verdict["witnesses"] and pack.get("public_key_hex"):
            # Two very different reasons land here, and reporting the wrong one sends the
            # reader to install a library they already have. Measured: pointed at the
            # published conformance vectors with cryptography>=48 installed, every
            # non-pack artifact (trust list, ID token, revocation feed) was reported as
            # "no ML-DSA verifier installed".
            if not any(_provider_available(n) for n in REAL_PROVIDERS):
                reason = "no ML-DSA verifier installed"
            else:
                reason = "not an authenticity pack (%s)" % (verdict["note"] or "no witness ran")
            print("%-34s SKIPPED (%s)" % (os.path.basename(p), reason))
            all_ok = False
            continue
        ok = (got == expect)
...
#: The crypto mode a run declares. Section 5 of the product contract: polaris-verify
#: refuses production startup unless one of these is EXPLICIT. There is no default and no
#: environment variable that selects one, because the failure this prevents is a verifier
#: that silently stopped doing real cryptography and kept answering.
CRYPTO_DEV_PLACEHOLDER = "DEV-PLACEHOLDER"

#: Real ML-DSA backends, in the order `auto` prefers them. Both are genuine
#: implementations: liboqs is the reference one, and cryptography>=48 carries ML-DSA
#: through OpenSSL. `oqs` alone would be a provider a clean `pip install` cannot get.
REAL_PROVIDERS = ("oqs", "cryptography")
...
def _provider_available(name):
    ...
    if name == "cryptography":
        try:
            ...
            return False
        # Present but too old to carry ML-DSA is the case worth catching: the import
        # succeeds and every verification would report "no verifier available".
        return hasattr(mldsa, "MLDSA65PublicKey")
    return False


def resolve_crypto_mode(provider, dev_placeholder):
    """Decide the declared crypto mode, or say why startup must be refused.

    Returns `(mode, error)`. Exactly one is None. `mode` is the string stamped onto every
    verdict and log line: a real provider's name, or DEV-PLACEHOLDER.

    Three refusals, and each is a real way a verifier ends up not verifying:

    1. Neither flag given. The caller has not said what they are running, so a machine
       with no ML-DSA library would answer every question with "cannot authenticate" and
       look like a working verifier that keeps finding nothing wrong.
    2. Both given. A run cannot be production and development at once.
    3. A real provider named but absent. Refusing beats starting and reporting a
       verification failure for every credential, which reads as the credentials being
       bad rather than the verifier being unable.
    """
    if provider and dev_placeholder:
        return None, ("--pqc-provider and --dev-placeholder are mutually exclusive: a run is "
                      "either doing real cryptography or explicitly not doing it")
    if not provider and not dev_placeholder:
        return None, ("refusing to start: say what cryptography this run uses.\n"
                      ...
                      "There is no default and no environment variable for this. A verifier "
                      "that quietly stopped verifying still answers every question.")
    if dev_placeholder:
        return CRYPTO_DEV_PLACEHOLDER, None
    if provider == "auto":
        for name in REAL_PROVIDERS:
            if _provider_available(name):
                return name, None
        return None, ("refusing to start: --pqc-provider auto found no real ML-DSA backend. "
                      "Install one (`pip install oqs` with liboqs, or `pip install "
                      "'cryptography>=48'`), or run with --dev-placeholder and accept that "
                      "nothing is authenticated.")
    if not _provider_available(provider):
        return None, ("refusing to start: --pqc-provider %s, but that backend is not usable "
                      "here. Install it, choose another, or run with --dev-placeholder and "
                      "accept that nothing is authenticated." % provider)
    return provider, None


def stamp_crypto(verdict, mode):
    """Put the declared crypto mode on a verdict, so a response can never be read without it.

    The contract asks for this on every relevant log line and verification response. A
    verdict that says `authentic: false` means something different when the reason is a bad
    signature than when the reason is that this process has no cryptography, and the two
    were indistinguishable to a caller reading JSON.
    """
...
def main(argv=None):
    ...
    # machine's installed packages decide for them.
    ap.add_argument("--pqc-provider", choices=("oqs", "cryptography", "auto"),
                    help="the REAL ML-DSA backend this run must use; startup is refused if "
                         "it is not usable here")
    ap.add_argument("--dev-placeholder", action="store_true",
                    help="run with NO real cryptography: development only. Every verdict and "
                         "log line is stamped crypto=DEV-PLACEHOLDER")
    ...
    if _err:
        print("polaris-verify: %s" % _err, file=sys.stderr)
        return 4
    if _mode == CRYPTO_DEV_PLACEHOLDER:
        set_dev_placeholder_mode(True)
        print("polaris-verify: crypto=%s -- NOTHING VERIFIED HERE IS AUTHENTICATED. "
              "Development use only." % CRYPTO_DEV_PLACEHOLDER, file=sys.stderr)
    else:
        print("polaris-verify: crypto=%s" % _mode, file=sys.stderr)
