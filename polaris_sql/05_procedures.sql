...
-- 2026-09-25. The single-decision half of `polaris retention-set`, as a procedure. The CLI
-- superseded the current decision with a direct UPDATE, and the application role is refused
-- UPDATE on RetentionPolicy on purpose (09_grants.sql: superseding without the record of who
-- did it is what the grant exists to stop). The CLI connects as polaris_app, so the command
-- failed with "permission denied" in every deployment and passed its tests, which connect as
-- the owner. This does the supersede and the insert as the owner, after the same actor checks
-- the template procedure makes.
