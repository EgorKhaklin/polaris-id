-- 2026-09-25-003: a rule the database enforces holds for a bound operator too.
--
-- Seven routines enforce a rule by reading IdentityToken, which row-level security filters for
-- an operator bound to one authority. Under that binding the rows they had to see were hidden,
-- so the rule held only for unbound operators. Measured on rc.41 as polaris_app bound to
-- authority 1: uc9_initiate_recovery opened a recovery for a holder with an ACTIVE credential
-- issued by authority 3 (unbound, refused), and enforce_revocation_status let authority 3's
-- ACTIVE token onto the RevocationList, because the hidden row's NULL status made
-- `NULL NOT IN (...)` NULL, which an IF reads as false (unbound, refused). Each is now SECURITY
-- DEFINER with a pinned search_path, executable by the application role only, and the
-- revocation check refuses a NULL status. Bodies copied from 05_procedures.sql, 06_triggers.sql
-- and 12_v7_constraints.sql.
