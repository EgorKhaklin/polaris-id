-- 2026-09-24-013: the audit-of-record privilege boundary reaches the partitions, and the
-- lifecycle log is written only by the routines that record a lifecycle.
--
-- Two holes in the same boundary, both measured as polaris_app on rc.39:
--   1. 09_grants.sql revoked UPDATE and DELETE on the four partitioned event tables by their
--      PARENT names; every partition kept the blanket grant. With the purge carve-out's setting,
--      which any role can set, DELETE on a partition passed the trigger too: every row of
--      TokenLifecycleEvent, VerificationEvent, EnrollmentStatusEvent and AuthAuditLog deleted.
--   2. polaris_app held INSERT on TokenLifecycleEvent because its four writers ran as the
--      caller, so it could append a lifecycle event nothing did.
-- Bodies copied from 01_schema.sql, 05_procedures.sql and 06_triggers.sql, which carry the same
-- change for a fresh install; the reasoning is there.
