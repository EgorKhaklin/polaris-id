...
is in BCNF (proven in §6.5 of the report), implements **36 tables** (v9.341; a migrated deployment holds 43, with the
...
1. Creates all 36 tables (`01_schema.sql`)
...
| `01_schema.sql` | DDL: 36 tables (incl. TimestampLog, AuthorityKeyEvent, AuthCodeConsumed, ExchangeNonce, ExchangeReceiptLog, RelyingParty, IssuerDiscretionPolicy, EnrollmentStatusEvent, RecoveryRequest, TokenSignature, AnchorBatch, AgencyTrustAttestation, TokenStateEpoch, TokenStateEpochLeaf, DuressEvent, LifecycleArchiveCheckpoint, AppUser, AuthAuditLog, RetentionPolicy) |
