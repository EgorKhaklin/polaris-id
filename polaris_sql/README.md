...
is in BCNF (proven in §6.5 of the report), implements **41 tables** (v9.368; a migrated deployment holds 47, with the
...
1. Creates all 41 tables (`01_schema.sql`)
...
| `01_schema.sql` | DDL: 41 tables (incl. CardPersonalization, HolderKeyEvent, TimestampLog, AuthorityKeyEvent, AuthCodeConsumed, ExchangeNonce, ExchangeReceiptLog, RelyingParty, IssuerDiscretionPolicy, EnrollmentStatusEvent, RecoveryRequest, TokenSignature, AnchorBatch, AgencyTrustAttestation, TokenStateEpoch, TokenStateEpochLeaf, DuressEvent, LifecycleArchiveCheckpoint, AppUser, AuthAuditLog, RetentionPolicy) |
