...
is in BCNF (proven in §6.5 of the report), implements **43 tables** (v9.368; a migrated deployment holds 47, with the
...
1. Creates all 43 tables (`01_schema.sql`)
...
| `01_schema.sql` | DDL: 43 tables (incl. CardPersonalization, HolderKeyEvent, TimestampLog, AuthorityKeyEvent, AuthCodeConsumed, ExchangeNonce, ExchangeReceiptLog, RelyingParty, IssuerDiscretionPolicy, EnrollmentStatusEvent, RecoveryRequest, TokenSignature, AnchorBatch, AgencyTrustAttestation, TokenStateEpoch, TokenStateEpochLeaf, DuressEvent, LifecycleArchiveCheckpoint, AppUser, AuthAuditLog, RetentionPolicy) |
