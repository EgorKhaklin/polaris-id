...
is in BCNF (proven in §6.5 of the report), implements **37 tables** (v9.349; a migrated deployment holds 44, with the
...
1. Creates all 37 tables (`01_schema.sql`)
...
| `01_schema.sql` | DDL: 37 tables (incl. HolderKeyEvent, TimestampLog, AuthorityKeyEvent, AuthCodeConsumed, ExchangeNonce, ExchangeReceiptLog, RelyingParty, IssuerDiscretionPolicy, EnrollmentStatusEvent, RecoveryRequest, TokenSignature, AnchorBatch, AgencyTrustAttestation, TokenStateEpoch, TokenStateEpochLeaf, DuressEvent, LifecycleArchiveCheckpoint, AppUser, AuthAuditLog, RetentionPolicy) |
