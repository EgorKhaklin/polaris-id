...
is in BCNF (proven in §6.5 of the report), implements **44 tables** (v9.440; a migrated deployment holds 51, with the
...
1. Creates all 44 tables (`01_schema.sql`)
...
| `01_schema.sql` | DDL: 44 tables (incl. CardPersonalization, HolderKeyEvent, TimestampLog, AuthorityKeyEvent, AuthCodeConsumed, ExchangeNonce, ExchangeReceiptLog, RelyingParty, IssuerDiscretionPolicy, EnrollmentStatusEvent, RecoveryRequest, TokenSignature, AnchorBatch, AgencyTrustAttestation, TokenStateEpoch, TokenStateEpochLeaf, DuressEvent, LifecycleArchiveCheckpoint, AppUser, AuthAuditLog, RetentionPolicy) |
