...
-- P9.1 (v9.349): the holder key register is strictly append-only. A binding that could be
-- updated or deleted would let an operator replace the holder's key, which is the whole
...
-- No backfill here, unlike AgencyEvent above. The load order is 06_triggers.sql then
-- 10_auth.sql, so this recorder is already installed when the seed accounts are created
-- and writes their CREATED rows itself. The backfill that an EXISTING database needs
-- lives in migration 2026-09-12-003-app-user-events, which is where that case arises.
