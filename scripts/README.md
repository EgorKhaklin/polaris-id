...
| `polaris-verifier-differential.py` | The detached verifier and the Python SDK answer the same hostile battery: 340 comparisons over 10 shared functions, and any input where they give a different KIND of answer is a finding. A note may differ; a verdict may not | by hand, after touching either |
...
| `polaris-unread-signed-fields.py` | Which fields the verifier canonicalises (and so signs over) are compared to nothing. A signed field nothing reads is a promise the signature carries and the verifier ignores: this found `agent_algorithm` on 2026-09-17, the same shape as `valid_until`. Exits non-zero on a field nobody has declared a reason for, and on a declared reason that has gone stale; the eleven known ones are split into bound-by-a-stronger-sibling and signed-but-not-surfaced, each with its reason | by hand, after adding a signed field |
| `polaris-app-mutation-drill.py` | Is each refusal the Flask application makes tested? Every `if` answering 4xx has its condition replaced by `False` one at a time, and a refusal nothing turns red is a survivor. `--probe` then asks the route what it ANSWERS with the refusal off, so a survivor splits into masked (something else refuses the same input) and a hole (it now ACCEPTS). Two controls: the tests must be GREEN unmutated and RED with the control refusal off. Needs the suite environment (see its docstring); rewrites app.py in place and prints its own repair line | by hand, after touching a route; 253 s default, ~40 min with --all --probe |
| `polaris-constitution-mutation-drill.py` | Delete each of C1-C10's enforcement in turn and require that constraint's OWN named check to go red. The other drills ask this of a mechanism; this asks it of the ten claims the mechanisms serve. First run, 2026-09-17: nine held, C8 did not. Refuses a dirty tree and a tree already failing | by hand, after touching an enforcement surface; about 20s |
| `polaris-coverage-mutation-drill.py` | The mirror of every other mutation drill. They DELETE a mechanism and require a check to go red; this ADDS a member to a surface that lacks it, because the defect this tree keeps finding is a check whose "every" reaches only what it was handed (duress on four pages of many, C6 on one half, C8's regex, C1's fourteen tables against thirty-four). Nine sixteen surfaces, each asserted by its own check's name; surfaces closed by construction are declared and the construction verified, so the excuse cannot outlive it. Refuses a dirty tree and a tree already failing | in CI on every push; about 20s |
| `polaris-move-mutation-drill.py` | The third question. The other drills hold a mechanism's FILE fixed: one deletes it in place, one adds a bad member beside it. This writes a violation that IS caught in app.py into a new sibling module and requires the same check to catch it there. Measured before the widening: 3 of 3 went blind, because 77 checks named app.py by path. The instrument that makes decomposing app.py a refactor rather than a bet | in CI on every push; about 20s |
