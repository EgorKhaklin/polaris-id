...
| `polaris-coverage-mutation-drill.py` | The mirror of every other mutation drill. They DELETE a mechanism and require a check to go red; this ADDS a member to a surface that lacks it, because the defect this tree keeps finding is a check whose "every" reaches only what it was handed (duress on four pages of many, C6 on one half, C8's regex, C1's fourteen tables against thirty-four). Nine surfaces, each asserted by its own check's name; surfaces closed by construction are declared and the construction verified, so the excuse cannot outlive it. Refuses a dirty tree and a tree already failing | in CI on every push; about 20s |
