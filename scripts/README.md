...
| `polaris-relying-party.py` | The other end of the flow: takes a holder's presentation and decides ACCEPT or REJECT by combining OFFLINE authenticity (the detached verifier, optionally against an issuer anchor) with the ONLINE status from `GET /verify`. Accept needs both; offline it is only PROVISIONAL. Standalone — only stdlib and the verifier, no Polaris code and no database | A relying party (a bank, a kiosk); `test_relying_party.py` under the coverage gate |
