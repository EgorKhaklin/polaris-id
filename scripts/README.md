...
| `polaris-app-mutation-drill.py` | Is each refusal the Flask application makes tested? Every `if` answering 4xx has its condition replaced by `False` one at a time, and a refusal nothing turns red is a survivor. Two controls: the tests must be GREEN unmutated and RED with the control refusal off. Needs the suite environment (see its docstring); rewrites app.py in place and prints its own repair line | by hand, after touching a route; 253 s for the 37 security-relevant refusals |
