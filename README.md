...
| [`polaris_checks/`](polaris_checks/) | The invariant layer. 286 checks (v1.0.0-rc.3), each with a tested failure mode. `python3 -m polaris_checks.run` gates CI. |
...
| Invariant checks | 286 | C1-C10 plus production posture, each check paired with a detection test |
| CI jobs | 20 | Below |
