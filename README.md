...
Each guarantee is machine-checked by [`polaris_checks`](polaris_checks/): 157 plain `check_*` functions (v9.291), each paired with a detection test proving it fails on a broken fixture. A check that cannot detect its own violation is treated as broken. The reasoning for why these ten, and why they interlock, is in [MISSION.md](MISSION.md) and [meta/constraint-lattice.md](meta/constraint-lattice.md).
...
| [`polaris_checks/`](polaris_checks/) | The invariant layer. 157 checks (v9.291), each with a tested failure mode. `python3 -m polaris_checks.run` gates CI. |
...
| Invariant checks | 157 | C1-C10 plus production posture, each check paired with a detection test |
