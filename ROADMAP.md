...
**Have, working, CI-proven:** a 35-table constraint-enforced schema (42 tables
in a migrated deployment) with append-only audit; a 114-route application with
...
release; CVE gates on dependencies and images; a coverage floor; 189 invariant
checks (v9.337) each with a detection test; eighteen operator runbooks and ledgers; the protocol layer
...
(the conformance suite is the contract; the integration itself has not happened); timestamp
transparency anchoring (P8.5b, a design decision not yet taken); a relying-party-signed
...
| [ ] P8.5b | Timestamp transparency: anchor each timestamp's digest in an append-only log | M | low | P8.5, P8.2c | v9.334 made long-term validity require a trusted, signer-independent timestamp authority; what remains is a timestamp authority whose OWN key is later compromised manufacturing backdated timestamps. Anchoring SHA3-256(timestamp) in an append-only transparency log with witnesses (the receipt log's mechanism, P8.2c) would make that visible, logging no document. Trade-off to decide first: the timestamp authority today learns and retains nothing (digest-only, unlogged, by design); anchoring retains one digest and one instant per timestamp. A design decision, not yet taken. |
